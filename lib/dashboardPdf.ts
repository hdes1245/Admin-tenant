...
const CHART_COLORS: [number,number,number][] = [C.navy, C.steel, C.gold, C.green, C.red, C.orange, C.violet];
...
function rgb(doc: jsPDF, col: [number,number,number], target: "fill"|"draw"|"text") {
...
function fr(doc: jsPDF, x:number,y:number,w:number,h:number,col:[number,number,number]) {
  rgb(doc,col,"fill"); doc.rect(x,y,w,h,"F");
}
function txt(doc:jsPDF,t:string,x:number,y:number,size:number,col:[number,number,number],align:"left"|"center"|"right"="left",bold=false) {
  doc.setFontSize(size); doc.setFont("helvetica",bold?"bold":"normal");
  rgb(doc,col,"text"); doc.text(t,x,y,{align});
}
function hline(doc:jsPDF,x1:number,y1:number,x2:number,y2:number,col:[number,number,number],lw=0.3) {
  rgb(doc,col,"draw"); doc.setLineWidth(lw); doc.line(x1,y1,x2,y2);
}

// ─── Pie/Donut sector drawing ─────────────────────────────────────────────────
function drawSector(doc:jsPDF,cx:number,cy:number,r:number,startDeg:number,endDeg:number,col:[number,number,number]) {
  ...
  for(let a=startDeg;a<endDeg;a+=step){
    ...
    // draw triangle as closed polygon via lines
    doc.lines(
...
function drawPieDonut(doc:jsPDF,data:{label:string;value:number}[],cx:number,cy:number,r:number,innerR:number,legendX:number,legendY:number) {
  ...
  data.forEach((d,i)=>{
    ...
    txt(doc,`${statusLabel(d.label)} (${d.value})`,legendX+5.5,ly,7.5,C.navy);
  });
}

// ─── Vertical bar chart ───────────────────────────────────────────────────────
function drawBarChart(doc:jsPDF,data:{label:string;value:number}[],x:number,y:number,w:number,h:number,col:[number,number,number]) {
...
function drawGroupedBar(doc:jsPDF,data:{label:string;a:number;b:number;c?:number}[],x:number,y:number,w:number,h:number,labels:[string,string,string?],colors:[[number,number,number],[number,number,number],[number,number,number]?]) {
  if(!data.length) return;
...
function drawHorizBar(doc:jsPDF,data:{label:string;value:number}[],x:number,y:number,w:number,col:[number,number,number]) {
  if(!data.length) return;
  const maxV=Math.max(...data.map(d=>d.value),1);
  const rowH=7;
...
// ─── Line chart ───────────────────────────────────────────────────────────────
function drawLineChart(doc:jsPDF,series:{label:string;col:[number,number,number];data:number[]}[],labels:string[],x:number,y:number,w:number,h:number) {
...
// ─── Progress bars ────────────────────────────────────────────────────────────
function drawProgressBars(doc:jsPDF,data:{label:string;pct:number;count:number;col:[number,number,number]}[],x:number,y:number,w:number) {
  const rowH=10;
  data.forEach((d,i)=>{
    const ry=y+i*rowH;
...
// ─── Section header ───────────────────────────────────────────────────────────
function secHdr(doc:jsPDF,title:string,y:number,pageW:number) {
...
// ─── Page header ─────────────────────────────────────────────────────────────
function pageHdr(doc:jsPDF,sectionTitle:string,page:string,tenant:string,pageW:number) {
  fr(doc,0,0,pageW,14,C.navy);
  fr(doc,0,0,pageW,3,C.gold);
  txt(doc,`TABLEAU DE BORD  ·  ${sectionTitle}`,14,10,8.5,C.gray,"left",true);
  txt(doc,`${page}  ·  ${tenant}`,pageW-14,10,8.5,[148,163,184],"right");
}

// ─── Simple table ─────────────────────────────────────────────────────────────
function simpleTable(doc:jsPDF,headers:string[],rows:string[][],sy:number,colW:number[],pageW:number,pageH:number):number {
...
// ─── Page footer ──────────────────────────────────────────────────────────────
function pageFtr(doc:jsPDF,tenant:string,period:string,pageW:number,pageH:number) {
...
// ─── KPI block — reproduit le style de la page web ────────────────────────────
// col1 = couleur foncée (haut), col2 = couleur claire (bas du dégradé simulé)
function kpiBlock(
  ...
  const c2 = col2 ?? col1;
  // Simule le dégradé avec 6 bandes horizontales de plus en plus claires
  const steps = 6;
  ...
  // Barre dorée en haut du bloc (accent, remplace les cercles décoratifs)
  ...
  // Sous-titre lisible (rgba simulé sur fond foncé → gris-bleu clair)
...
export function generateDashboardPdf(data: PdfReportData) {
  ...
  txt(doc,`Généré le ${fmt(new Date().toISOString())}`,W/2,138,9,[100,116,139],"center");
  ...
  // ══════════════════════════════════════════════════════════════════════════════
  doc.addPage();
  pageHdr(doc,"KPIs","Page 2",data.tenantName,W);
  ...
  // ══════════════════════════════════════════════════════════════════════════════
  doc.addPage();
  pageHdr(doc,"ACTIVITÉ / JOUR","Page 3",data.tenantName,W);
  pageFtr(doc,data.tenantName,data.period,W,H);
  y=22;
  ...
  // ══════════════════════════════════════════════════════════════════════════════
  doc.addPage();
  pageHdr(doc,"TICKETS & ACTIONS","Page 4",data.tenantName,W);
  pageFtr(doc,data.tenantName,data.period,W,H);
  y=22;
  ...
  if(data.ticketsByStatus.length>0){
    ...
    drawPieDonut(doc,pieData,cx,cy,36,16,M+halfW+8,y+6);
    y+=90;
  } else {
  ...
  // ══════════════════════════════════════════════════════════════════════════════
  doc.addPage();
  pageHdr(doc,"CLIENTS","Page 5",data.tenantName,W);
  pageFtr(doc,data.tenantName,data.period,W,H);
  y=22;
  ...
  // ══════════════════════════════════════════════════════════════════════════════
  doc.addPage();
  pageHdr(doc,"RESSOURCES","Page 6",data.tenantName,W);
  pageFtr(doc,data.tenantName,data.period,W,H);
  y=22;
  ...
  // ══════════════════════════════════════════════════════════════════════════════
  doc.addPage();
  pageHdr(doc,"ZONES","Page 7",data.tenantName,W);
  pageFtr(doc,data.tenantName,data.period,W,H);
  y=22;
